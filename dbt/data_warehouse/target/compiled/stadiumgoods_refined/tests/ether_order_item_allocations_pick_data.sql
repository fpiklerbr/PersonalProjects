...
SELECT 
  *
FROM 
  `ff-stadiumgoods-refined-live`.`stadiumgoods_staging`.`stg_x3_order_lines_picks` x3 
LEFT JOIN 
  `ff-stadiumgoods-refined-live`.`stadiumgoods_staging`.`stg_ether_order_item_allocations` ether
ON 
