    ...
    merge into `ff-stadiumgoods-refined-live`.`stadiumgoods_staging`.`stg_ether_inventory_unit_price_history` as DBT_INTERNAL_DEST
        using (
...
SELECT * FROM joins
    ...
    when not matched then insert
        (`ether_wms_inventory_unit_version_id`, `ether_wms_inventory_unit_id`, `serial_number`, `action_type`, `object_type`, `previous_price_base_amount`, `price_base_amount`, `valid_from`, `action_date`, `user_id`, `employee_id`)
    values
        (`ether_wms_inventory_unit_version_id`, `ether_wms_inventory_unit_id`, `serial_number`, `action_type`, `object_type`, `previous_price_base_amount`, `price_base_amount`, `valid_from`, `action_date`, `user_id`, `employee_id`)
