    ...
    merge into `ff-stadiumgoods-refined-live`.`stadiumgoods_staging`.`stg_ether_inventory_unit_stock_status_history` as DBT_INTERNAL_DEST
        using (
...
SELECT * FROM joins
    ...
    when not matched then insert
        (`ether_wms_inventory_unit_version_id`, `ether_wms_inventory_unit_id`, `action_type`, `object_type`, `previous_stock_status_code`, `stock_status_code`, `logged_at`, `user_id`, `employee_id`)
    values
        (`ether_wms_inventory_unit_version_id`, `ether_wms_inventory_unit_id`, `action_type`, `object_type`, `previous_stock_status_code`, `stock_status_code`, `logged_at`, `user_id`, `employee_id`)
