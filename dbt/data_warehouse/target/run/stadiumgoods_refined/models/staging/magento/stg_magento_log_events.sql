    ...
    merge into `ff-stadiumgoods-refined-live`.`stadiumgoods_staging`.`stg_magento_log_events` as DBT_INTERNAL_DEST
        using (
...
from parsed_messages
    ...
    when matched then update set
        `serial_log_id` = DBT_INTERNAL_SOURCE.`serial_log_id`,`serial_number` = DBT_INTERNAL_SOURCE.`serial_number`,`serial_creation_utc` = DBT_INTERNAL_SOURCE.`serial_creation_utc`,`log_timestamp_utc` = DBT_INTERNAL_SOURCE.`log_timestamp_utc`,`log_event` = DBT_INTERNAL_SOURCE.`log_event`,`_synched_from_source_at` = DBT_INTERNAL_SOURCE.`_synched_from_source_at`
    

    when not matched then insert
        (`serial_log_id`, `serial_number`, `serial_creation_utc`, `log_timestamp_utc`, `log_event`, `_synched_from_source_at`)
    values
        (`serial_log_id`, `serial_number`, `serial_creation_utc`, `log_timestamp_utc`, `log_event`, `_synched_from_source_at`)
