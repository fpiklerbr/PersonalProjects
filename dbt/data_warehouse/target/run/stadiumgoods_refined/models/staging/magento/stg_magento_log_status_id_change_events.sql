  ...
  create or replace table `ff-stadiumgoods-refined-live`.`stadiumgoods_staging`.`stg_magento_log_status_id_change_events`
  ...
  as (
    ...
    (
        ...
        WHERE COALESCE(old_status_id, -1) <> COALESCE(status_id, -1) 
    ),
    ordered_status_id_changes as 
    (
        SELECT serial_log_id
    ...
    (
        ...
        FROM ordered_status_id_changes
    ),
    lookback_rows as 
    (
        SELECT serial_log_id
            ...
            , status_id 
            , case 
                when serial_row_number = 10 then old_status_id
                else lag(status_id) OVER (ORDER BY serial_number, serial_row_number) 
            end as prev_status_id
    ...
    (
        ...
        FROM lookback_rows 
        where old_status_id <> prev_status_id
    ),
    artifical_rows as 
    (
 ...
 AS datetime), 'Etc/UTC') AS timestamp) as log_timestamp_utc
            , prev_status_id as old_status_id 
            , old_status_id as status_id 
            , NULL as log_event
    ...
    (
        ...
        SELECT serial_log_id
            ...
            , serial_creation_utc
            , log_timestamp_utc
            , COALESCE(old_status_id
                , lag(status_id) OVER 
                    (ORDER BY serial_number, serial_row_number)) as old_status_id
            , status_id
            , log_event
    ...
    SELECT *
    FROM expanded_status_id_changes
