  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`ether__commercial_purchase_order_notes`
  OPTIONS()
  as 
 ...
 as int64)
 AS `ether_commercial_purchase_order_note_id`
  , 
...
FROM `ff-stadiumgoods-raw-live`.`stadium_goods_production`.`ether_commercial_purchase_order_notes`

WHERE 1=1
                 ) , final AS ( 
                    SELECT

    `ether_commercial_purchase_order_note_id` AS `ether_commercial_purchase_order_note_id`
