  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`cap__shipping`
  OPTIONS()
  as 
 ...
 AS `capacity_uom`
  , 
    ...
    safe_cast(
    safe_cast(CASE WHEN cast(nullif(trim(cast(`capacity_case_pack` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`capacity_case_pack` AS string)), '') AS string) END as float64)
 as int64)
 ...
 AS `tracking_number`
  , 
    ...
    safe_cast(CASE WHEN cast(nullif(trim(cast(`ship_weight` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`ship_weight` AS string)), '') AS string) END as float64)
 AS `ship_weight`
  , 

    
    safe_cast(
    safe_cast(CASE WHEN cast(nullif(trim(cast(`ship_cartons` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`ship_cartons` AS string)), '') AS string) END as float64)
 as int64)
 AS `ship_cartons`
  , 
 ...
 AS `file_name`
  , 
    ...
    safe_cast(CASE WHEN cast(nullif(trim(cast(`shipping_timestamp` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`shipping_timestamp` AS string)), '') AS string) END as timestamp)
...
WHERE 1=1
    ...
    *
  , 
    safe_cast(coalesce(_raw_po_number, ponumber) as string)
 AS `po_number`
  , 
    safe_cast(shipping_timestamp as timestamp)
 AS `file_time`
  , 
    safe_cast(shipping_timestamp as timestamp)
 AS `shipped_at`
  , 
    safe_cast('CAP' as string)
 AS `ship_warehouse`
  , 
    safe_cast(1 as int64)
 AS `warehouse_priority`
FROM layer_0
WHERE 1=1
                     ) , final AS ( 
                    SELECT
  ...
  , `shipping_timestamp` AS `shipping_timestamp`
