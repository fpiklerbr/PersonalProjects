  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`law__shipping`
  OPTIONS()
  as 
 ...
 AS `file_time`
FROM `ff-stadiumgoods-raw-live`.`imports`.`law_shipping`
...
WHERE 1=1
    ...
    *
  , 
    safe_cast(CASE WHEN length(ship_tracking) = 34 THEN substr(ship_tracking, CHAR_LENGTH(ship_tracking) - (12 - 1), 12) ELSE ship_tracking END as string)
 AS `tracking_number`
  , 
    safe_cast(CASE WHEN length(ship_tracking) = 34 THEN ship_tracking END as string)
 AS `tracking_barcode`
  , 
    safe_cast(file_time as timestamp)
 AS `shipped_at`
  , 
    safe_cast('LAW' as string)
 AS `ship_warehouse`
  , 
    safe_cast(2 as int64)
...
WHERE 1=1
  ...
  , `ship_tracking` AS `ship_tracking`
  , `tracking_number` AS `tracking_number`
  , `tracking_barcode` AS `tracking_barcode`
