  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`law__receiving`
  OPTIONS()
  as 
 ...
 AS `capacity_po_number`
  ...
  regexp_contains(CASE WHEN cast(nullif(trim(cast(`receive_date` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`receive_date` AS string)), '') AS string) END, r'(?i)^[0-9]{1,2}[0-9]{1,2}[0-9]{4}$')


          THEN  parse_timestamp('%m%d%Y', CASE WHEN cast(nullif(trim(cast(`receive_date` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`receive_date` AS string)), '') AS string) END)
        END as timestamp)
 ...
 AS `file_time`
FROM `ff-stadiumgoods-raw-live`.`imports`.`law_receiving`
