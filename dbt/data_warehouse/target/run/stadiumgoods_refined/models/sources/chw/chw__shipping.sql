  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`chw__shipping`
  OPTIONS()
  as 
 ...
 AS `file_time`
FROM `ff-stadiumgoods-raw-live`.`imports`.`chw_shipping`
...
WHERE 1=1
 ...
 AS `shipped_at`
  , 
    safe_cast('CHW' as string)
 AS `ship_warehouse`
  , 
    safe_cast(3 as int64)
