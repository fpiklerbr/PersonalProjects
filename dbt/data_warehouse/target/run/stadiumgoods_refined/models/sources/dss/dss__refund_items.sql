  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`dss__refund_items`
  OPTIONS()
  as 
...
SELECT   `refund_item_id`,  `third_party_id`,  `refund_id`,  `order_id`,  `order_item_id`,  `quantity`,  `subtotal`,  `created_at`,  `updated_at`
...
SELECT
 ...
 AS `third_party_id`
  ...
  , 
    safe_cast(
    safe_cast(CASE WHEN cast(nullif(trim(cast(`refund_id` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`refund_id` AS string)), '') AS string) END as float64)
 as int64)
 AS `refund_id`
 ...
 AS `quantity`
  ...
  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`subtotal` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`subtotal` AS string)), '') AS string) END as string)
 AS `subtotal`
...
FROM `ff-stadiumgoods-raw-live`.`dss_portal_production`.`refund_items` AS source_table
