  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`alipay__tmall_settlement_fee`
  OPTIONS()
  as 
...
SELECT   `payment_time`,  `alipay_transaction_id`,  `tmall_order_id`,  `transaction_amount`,  `rmb_gross_amount`,  `fee_type`,  `fee_amount`,  `fee_amount_rmb`,  `currency`,  `_raw_exchange_rate`,  `fee_description`,  `file_time`,  `file_name`,  `original_tmall_order_id`,  `transaction_type`,  `is_shipping`,  `is_duties`,  `crossdock_reference`
  FROM ( 
...
FROM `ff-stadiumgoods-raw-live`.`imports`.`alipay_tmall_settlement_fee` AS source_table
