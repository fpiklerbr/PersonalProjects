  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`alipay__sg_settlebatch`
  OPTIONS()
  as 
...
FROM `ff-stadiumgoods-raw-live`.`imports`.`alipay_sg_settlebatch` AS source_table
