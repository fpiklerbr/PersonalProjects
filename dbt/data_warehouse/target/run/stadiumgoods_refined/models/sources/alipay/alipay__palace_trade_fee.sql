  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`alipay__palace_trade_fee`
  OPTIONS()
  as 
...
SELECT   `payment_time`,  `alipay_transaction_id`,  `tmall_order_id`,  `amount`,  `transaction_amount`,  `currency`,  `_raw_exchange_rate`,  `fee_type`,  `fee_amount_rmb`,  `fee_amount`,  `fee_description`,  `original_tmall_order_id`,  `file_time`,  `file_name`,  `transaction_type`,  `is_shipping`,  `is_duties`,  `crossdock_reference`
  FROM ( 
...
FROM `ff-stadiumgoods-raw-live`.`imports`.`tmall_palace_trade_fee` AS source_table
