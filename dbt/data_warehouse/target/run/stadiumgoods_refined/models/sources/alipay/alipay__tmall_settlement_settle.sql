  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`alipay__tmall_settlement_settle`
  OPTIONS()
  as 
...
SELECT   `partner_transaction_id`,  `amount`,  `rmb_amount`,  `fee`,  `settlement`,  `rmb_settlement`,  `currency`,  `rate`,  `payment_time`,  `settlement_time`,  `settlement_date`,  `transaction_description`,  `status`,  `stem_from`,  `remarks`,  `original_partner_transaction_id`,  `file_time`,  `file_name`,  `transaction_processed_at`,  `transaction_id`,  `settlement_id`,  `payment_amount`,  `refund_amount`,  `payment_fee`,  `refund_fee`,  `payment_processed_at`,  `refund_processed_at`
  FROM ( SELECT
    *
...
SELECT
 ...
 AS `transaction_id`
  , 
    safe_cast(CASE WHEN transaction_description = 'P' THEN amount END as float64)
 AS `payment_amount`
  , 
    safe_cast(CASE WHEN transaction_description = 'R' THEN (-1 * amount) END as float64)
 AS `refund_amount`
  , 
...
FROM `ff-stadiumgoods-raw-live`.`imports`.`alipay_tmall_settlement_settle` AS source_table
