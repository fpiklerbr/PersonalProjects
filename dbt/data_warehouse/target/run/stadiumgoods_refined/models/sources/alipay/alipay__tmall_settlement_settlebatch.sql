  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`alipay__tmall_settlement_settlebatch`
  OPTIONS()
  as 
...
FROM `ff-stadiumgoods-raw-live`.`imports`.`alipay_tmall_settlement_settlebatch` AS source_table
