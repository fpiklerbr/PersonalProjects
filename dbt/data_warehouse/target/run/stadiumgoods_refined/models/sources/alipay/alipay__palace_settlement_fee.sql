  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`alipay__palace_settlement_fee`
  OPTIONS()
  as 
...
SELECT   `payment_time`,  `alipay_transaction_id`,  `tmall_order_id`,  `transaction_amount`,  `rmb_gross_amount`,  `fee_type`,  `fee_amount`,  `fee_amount_rmb`,  `currency`,  `_raw_exchange_rate`,  `fee_description`,  `original_tmall_order_id`,  `file_time`,  `file_name`,  `_empty_transaction_amount_rmb`,  `_empty_transaction_amount_usd`,  `_empty_exchange_rate`,  `_empty_fee_amount_usd`,  `_empty_fee_description`,  `transaction_type`,  `is_shipping`,  `is_duties`,  `crossdock_reference`
  FROM ( 
...
SELECT
 ...
 AS `tmall_order_id`
  ...
  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`gross_amount` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`gross_amount` AS string)), '') AS string) END as float64)
 AS `transaction_amount`

  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`rmb_gross_amount` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`rmb_gross_amount` AS string)), '') AS string) END as string)
 AS `rmb_gross_amount`
 ...
 AS `_raw_fee_amount_usd`
  ...
  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`fee_rmb_amount` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`fee_rmb_amount` AS string)), '') AS string) END as float64)
 ...
 AS `_raw_exchange_rate`
  ...
  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`remark` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`remark` AS string)), '') AS string) END as string)
 AS `fee_description`
 ...
 AS `_empty_transaction_amount_rmb`

  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`foreign_amount` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`foreign_amount` AS string)), '') AS string) END as string)
 AS `_empty_transaction_amount_usd`
 ...
 AS `_empty_exchange_rate`

  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`foreign_fee_amount` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`foreign_fee_amount` AS string)), '') AS string) END as string)
 AS `_empty_fee_amount_usd`
 ...
 AS `_empty_fee_description`



FROM `ff-stadiumgoods-raw-live`.`imports`.`tmall_palace_settlement_fee` AS source_table
