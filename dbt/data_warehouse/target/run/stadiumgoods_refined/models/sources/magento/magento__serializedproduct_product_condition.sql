  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__serializedproduct_product_condition`
  OPTIONS()
  as 
...
SELECT   `serializedproduct_product_condition_id`,  `condition_name`,  `created_at`,  `updated_at`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`serializedproduct_product_condition` AS source_table
