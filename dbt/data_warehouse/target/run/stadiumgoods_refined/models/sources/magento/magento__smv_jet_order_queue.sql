  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__smv_jet_order_queue`
  OPTIONS()
  as 
...
SELECT   `smv_jet_order_queue_id`,  `merchant_order_id`,  `order_status`,  `status`,  `error_message`,  `created_at`,  `updated_at`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`smv_jet_order_queue` AS source_table
