  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__catalog_product_entity_datetime`
  OPTIONS()
  as 
...
SELECT   `catalog_product_entity_datetime_id`,  `entity_type_id`,  `attribute_id`,  `store_id`,  `product_id`,  `attribute_value`
...
SELECT
 ...
 as int64)
 AS `catalog_product_entity_datetime_id`
 ...
 AS `product_id`
  ...
  , SAFE_CAST(TIMESTAMP(SAFE_CAST(
    safe_cast(CASE WHEN cast(nullif(trim(cast(`value` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`value` AS string)), '') AS string) END as timestamp)
 AS datetime), 'Etc/UTC') AS timestamp) AS `attribute_value`


  , `_synched_from_source_at` AS `_raw__synched_from_source_at`


FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`catalog_product_entity_datetime` AS source_table
