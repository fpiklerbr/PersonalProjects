  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__serializedproduct_product_serial_status`
  OPTIONS()
  as 
...
SELECT   `status_id`,  `status_name`,  `created_at`,  `updated_at`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`serializedproduct_product_serial_status` AS source_table
