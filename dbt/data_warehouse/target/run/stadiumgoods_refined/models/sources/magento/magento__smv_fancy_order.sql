  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__smv_fancy_order`
  OPTIONS()
  as 
...
SELECT   `smv_fancy_order_id`,  `fancy_order_id`,  `sales_flat_order_id`,  `created_at`,  `updated_at`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`smv_fancy_order` AS source_table
