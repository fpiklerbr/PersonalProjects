  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__catalog_product_entity_varchar`
  OPTIONS()
  as 
...
SELECT   `catalog_product_entity_varchar_id`,  `entity_type_id`,  `attribute_id`,  `store_id`,  `product_id`,  `attribute_value`
...
SELECT
 ...
 as int64)
 AS `catalog_product_entity_varchar_id`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`catalog_product_entity_varchar` AS source_table
