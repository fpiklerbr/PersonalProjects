  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__serializedproduct_product_serial_warehouse`
  OPTIONS()
  as 
...
SELECT   `warehouse_id`,  `warehouse_name`,  `created_at`,  `updated_at`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`serializedproduct_product_serial_warehouse` AS source_table
