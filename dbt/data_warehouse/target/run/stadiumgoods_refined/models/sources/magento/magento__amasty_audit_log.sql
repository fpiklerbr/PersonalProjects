    ...
    merge into `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__amasty_audit_log` as DBT_INTERNAL_DEST
        using (
...
WHERE 1=1
    ...
    when not matched then insert
        (`amasty_audit_log_id`, `log_time`, `username`, `action_type`, `category`, `category_name`, `parameter_name`, `element_id`, `info`, `store_id`, `_synched_from_source_at`)
    values
        (`amasty_audit_log_id`, `log_time`, `username`, `action_type`, `category`, `category_name`, `parameter_name`, `element_id`, `info`, `store_id`, `_synched_from_source_at`)
