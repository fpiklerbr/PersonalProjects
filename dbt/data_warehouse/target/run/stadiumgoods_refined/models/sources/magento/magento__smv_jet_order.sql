  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__smv_jet_order`
  OPTIONS()
  as 
...
SELECT   `smv_jet_order_id`,  `jet_order_id`,  `merchant_order_id`,  `sales_flat_order_id`,  `created_at`,  `updated_at`
...
SELECT
 ...
 AS `smv_jet_order_id`
  ...
  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`jet_order_id` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`jet_order_id` AS string)), '') AS string) END as string)
 AS `jet_order_id`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`smv_jet_order` AS source_table
