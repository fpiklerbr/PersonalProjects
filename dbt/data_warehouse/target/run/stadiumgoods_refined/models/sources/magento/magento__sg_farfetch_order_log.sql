  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__sg_farfetch_order_log`
  OPTIONS()
  as 
...
SELECT   `entity_id`,  `order_id`,  `farfetch_order_id`,  `message`,  `type`,  `created_at`,  `raw_order_data`,  `_synched_from_source_at`
  FROM ( 
...
SELECT
 ...
 AS `order_id`
  ...
  , 
    safe_cast(
    safe_cast(CASE WHEN cast(nullif(trim(cast(`farfetch_order_id` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`farfetch_order_id` AS string)), '') AS string) END as float64)
 as int64)
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`sg_farfetch_order_log` AS source_table
