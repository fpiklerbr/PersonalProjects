  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__catalog_product_index_eav`
  OPTIONS()
  as 
...
SELECT   `product_id`,  `attribute_id`,  `store_id`,  `option_id`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`catalog_product_index_eav` AS source_table
