  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`magento__catalog_product_entity_decimal`
  OPTIONS()
  as 
...
SELECT   `catalog_product_entity_decimal_id`,  `entity_type_id`,  `attribute_id`,  `store_id`,  `product_id`,  `attribute_value`
...
SELECT
 ...
 as int64)
 AS `catalog_product_entity_decimal_id`
 ...
 AS `product_id`
  ...
  , 
    safe_cast(CASE WHEN cast(nullif(trim(cast(`value` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`value` AS string)), '') AS string) END as numeric)
 AS `attribute_value`
...
FROM `ff-stadiumgoods-raw-live`.`stadium_production`.`catalog_product_entity_decimal` AS source_table
