    ...
    merge into `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`barrett__inventory` as DBT_INTERNAL_DEST
        using (
...
FROM `ff-stadiumgoods-raw-live`.`imports`.`barrett_inventory`
...
AND 
    ...
    safe_cast(CASE WHEN cast(nullif(trim(cast(`file_time` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`file_time` AS string)), '') AS string) END as timestamp)
 > (SELECT max(file_time) FROM `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`barrett__inventory`)
                 ) , final AS ( 
                    SELECT
  ...
  , `inventory_file` AS `inventory_file`
...
WHERE 1=1
    ...
    when not matched then insert
        (`label_number`, `alt_sku`, `description`, `qty_on_hand`, `commited`, `available`, `price`, `category`, `file_name`, `inventory_file`, `file_time`)
    values
        (`label_number`, `alt_sku`, `description`, `qty_on_hand`, `commited`, `available`, `price`, `category`, `file_name`, `inventory_file`, `file_time`)
