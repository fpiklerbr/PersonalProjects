  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`seller_portal__countries`
  OPTIONS()
  as 
...
SELECT   `country_id`,  `country_code`,  `country_name`,  `last_modified_in_x3_at`,  `created_at`,  `updated_at`,  `region_code`,  `_synched_from_source_at`
  FROM ( 
...
FROM `ff-stadiumgoods-raw-live`.`stadium_goods_production`.`countries` AS source_table
