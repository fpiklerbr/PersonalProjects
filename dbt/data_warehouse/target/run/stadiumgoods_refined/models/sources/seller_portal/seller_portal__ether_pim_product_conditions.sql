  ...
  create or replace view `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`seller_portal__ether_pim_product_conditions`
  OPTIONS()
  as 
 ...
 as int64)
 AS `ether_pim_product_condition_id`
  , 
...
FROM `ff-stadiumgoods-raw-live`.`stadium_goods_production`.`product_conditions`

WHERE 1=1
                 ) , final AS ( 
                    SELECT

    `ether_pim_product_condition_id` AS `ether_pim_product_condition_id`
