  ...
  create or replace table `ff-stadiumgoods-refined-live`.`stadiumgoods_sources`.`farfetch__financial_refunds`
 ...
 / -1 AS `refund_item_price_and_tax_base_amount`
  , 
    ...
    safe_cast(CASE WHEN cast(nullif(trim(cast(`net_sales_price_a1_` AS string)), '') AS string) IN ('nan','None','','NaT') THEN NULL ELSE cast(nullif(trim(cast(`net_sales_price_a1_` AS string)), '') AS string) END as numeric)
 ...
 AS `refund_file_time`
FROM `ff-stadiumgoods-raw-live`.`imports`.`farfetch_financial_refunds`
