  ...
  create or replace table `ff-stadiumgoods-refined-live`.`stadiumgoods_reporting`.`stadium_finance_line_transactions`
...
-- depends_on: `ff-stadiumgoods-refined-live`.`stadiumgoods_reporting`.`stadium_finance_lines`
...
, customer_city) END AS us_city
  ...
  r'USD'
 AS currency
    , round(order_serial_price, 2) AS order_serial_price
    , round(line_item_price_before_upcharge, 2) AS item_price_before_upcharge
    , round(line_item_upcharge, 2) AS channel_upcharge
    , (
        round(line_item_price_paid, 2)
        + round(line_item_store_credit_applied, 2)
      ) AS item_price_paid
    , -1 * round(line_item_discount, 2) AS item_discount
    , (
        round(line_shipping_price_paid, 2)
        + round(line_shipping_store_credit_applied, 2)
      ) AS shipping_price_paid
    , -1 * round(line_shipping_discount, 2) AS shipping_discount
    , ( round(line_item_tax_paid, 2) +
        CASE WHEN business_entity = 'STADIUM GOODS EU' THEN 0
        ELSE round(line_shipping_tax_paid, 2)
        END ) AS tax_paid
    , -1 * (
        round(line_item_price_refunded, 2)
        + round(line_item_price_credited, 2)
      ) AS item_price_refunded
    , -1 * (
        round(line_shipping_price_refunded, 2)
        + round(line_shipping_price_credited, 2)
      ) AS shipping_price_refunded
    , -1 * (
        round(line_item_tax_refunded, 2)
        + round(line_item_tax_credited, 2)
        + CASE WHEN business_entity = 'STADIUM GOODS EU' THEN 0
            ELSE (round(line_shipping_tax_refunded, 2)+ round(line_shipping_tax_credited, 2))
            END
      ) AS tax_refunded
    , round(return_refund_expected, 2) AS return_refund_expected
    , -1 * round(serial_cost, 2) AS serial_cost
    , round(payment_total_shipping_subsidy, 2) AS payment_total_shipping_subsidy
    , round(line_payment_shipping_subsidy, 2) AS line_payment_shipping_subsidy
    , round(refund_total_shipping_subsidy, 2) AS refund_total_shipping_subsidy
    , round(line_refund_shipping_subsidy, 2) AS line_refund_shipping_subsidy
    , fps_stock_point_id
    , business_entity
...
FROM finance_lines
  ...
  WHERE transaction_date BETWEEN '2023-12-01' AND '2023-12-21'
...
SELECT
 ...
 AS date) AS transaction_month
  , transaction_date BETWEEN '2023-12-01' AND '2023-12-21' AS transaction_is_in_reporting_month
  ...
  , cast('2023-12-21' AS date) AS month_ending
  , cast('2023-12-21' AS date) AS snapshot_date
